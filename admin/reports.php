...
<div id="tpcmem-report-chart" style="width: 100%; height: 300px;"></div>
<?php

$args = $checkpointAction ? array('checkpoint_action' => $checkpointAction) : '';
$rawChartData = array();
$chartData = array();

foreach (tpcmem_get_usage_records($args) as $record) {
    $action = $record->checkpoint_action;
    if (!array_key_exists($action, $rawChartData)) {
        $rawChartData[$action] = array();
    }

    $rawChartData[$action][] = array(
        // JavaScript timestamps are measured in milliseconds
        get_date_from_gmt($record->time, 'U') * 1000,
        tpcmem_bytes_to_mb($record->usage),
    );
}

foreach ($rawChartData as $action => $data) {
    $chartData[] = array(
        'label' => $action,
        'data'  => $data,
    );
