...
function tpcmem_display_dashboard_widget() {
    ...
    $memoryGraphClass = 'tpcmem-memory-';
    if ($gPercent > 90) {
        $memoryGraphClass .= 'high';
    } elseif ($gPercent > 70) {
        $memoryGraphClass .= 'medium';
    } else {
        $memoryGraphClass .= 'low';
    }
?>
<ul id="tpcmem-dashboard-data">
    <li>
        <strong><?php esc_html_e('Usage Sample'); ?></strong>:
        <span><?php echo esc_html( tpcmem_bytes_to_mb($current) . 'MB' ); ?></span>
        <span class="<?php echo esc_attr($memoryGraphClass); ?>">(<?php echo $currentPercentage; ?>%)</span>
    </li>
    <li>
        <strong><?php esc_html_e('Peak Usage'); ?></strong>:
        <span><?php echo esc_html( tpcmem_bytes_to_mb($peak) . 'MB' ); ?></span>
        <span class="<?php echo esc_attr($memoryGraphClass); ?>">(<?php echo $peakPercentage; ?>%)</span>
    </li>
...
<div class="progressbar">
    <div id="tpcmem_pbar_wrap">
    <div id="tpcmem_pbar" class="<?php echo esc_attr($memoryGraphClass); ?>" style="width: <?php echo $gPercent; ?>%;">
...
function tpcmem_dropdown_items_per_page($selected = false) {
    foreach (array(10, 25, 50, 100, 200) as $numItems) {
        if ($selected == $numItems)
            $default = "<option value='{$numItems}' selected='selected'>{$numItems} per page</option>\r\n";
        else
            $options .= "<option value='{$numItems}'>{$numItems}</option>\r\n";
    }
...
function tpcmem_dropdown_checkpoint_actions($selected = false) {
    ...
    foreach ($actions as $action) {
        ...
        if ($selected === $action)
            $d = "<option value='{$action}' selected='selected'>{$action}</option>\r\n";
        else
            $o .= "<option value='{$action}'>{$action}</option>\r\n";
    }
